				...
				foreach ($this->tokenValidators as $validatorClass) {
					...
					if ($tokenUserId) {
						...
						if ($autoProvisionAllowed) {
							...
							/** @psalm-suppress UndefinedClass Only available on NC 34 or above */
							try {
								$existingUser = $this->lookupUserInLDAP($provider, $tokenUserId);
							} catch (MultipleUsersReturnedException) {
	...
	private function lookupUserInLDAP(Provider $provider, string $tokenUserId): ?IUser {
		...
		if ($this->ldapService->isLDAPEnabled()) {
			...
			if ($ldapMappingAttribute !== '') {
				...
				$existingUserId = $this->ldapService->findUserByAttribute($ldapMappingAttribute, $tokenUserId);
				if ($existingUserId !== null) {
					$existingUser = $this->userManager->get($existingUserId);
				}
