	...
	public function changeSchema(IOutput $output, Closure $schemaClosure, array $options) {
		...
		if ($schema->hasTable('junovy_user_oidc_sessions')) {
			...
			foreach ($indexes as $index) {
				...
				if ($index->isUnique() && $index->hasColumnAtPosition('sid') && $index->getName() !== 'junovy_user_oidc_sess_sid') {
					$table->renameIndex($index->getName(), 'junovy_user_oidc_sess_sid');
					$somethingChanged = true;
				}
				if ($index->isUnique() && $index->hasColumnAtPosition('nc_session_id') && $index->getName() !== 'junovy_user_oidc_sess_sess_id') {
					$table->renameIndex($index->getName(), 'junovy_user_oidc_sess_sess_id');
					$somethingChanged = true;
				}
			}
		}
		...
		if ($schema->hasTable('junovy_user_oidc_providers')) {
			...
			foreach ($indexes as $index) {
				...
				if ($index->isUnique() && $index->hasColumnAtPosition('identifier') && $index->getName() !== 'junovy_user_oidc_prov_idtf') {
					$table->renameIndex($index->getName(), 'junovy_user_oidc_prov_idtf');
					$somethingChanged = true;
				}
			}
		}
