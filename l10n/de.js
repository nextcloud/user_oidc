OC.L10N.register(
    ...
    "Failed to authenticate" : "Anmeldung fehlgeschlagen",
    ...
    "Use an OpenID Connect backend to login to your Nextcloud" : "Ein OpenID Connect-Backend verwenden, um sich bei deiner Nextcloud anzumelden",
    ...
    "Store login tokens" : "Store-Login-Token",
    "\"Store login tokens\" is needed if you are using other apps that want to use user_oidc's token exchange or simply get the login token" : "\"Login-Token speichern\" wird benötigt, wenn andere Apps verwendet werden, die den Token-Austausch von user_oidc nutzen oder einfach das Login-Token erhalten möchten",
    ...
    "Post logout URI" : "URI nach der Abmeldung",
    ...
    "Birth date mapping" : "Zuordnung des Geburtsdatums",
    "Pronouns mapping" : "Pronomenzuordnung",
    ...
    "Locale mapping" : "Gebietszuordnung",
    ...
    "X (formerly Twitter) mapping" : "X (früher Twitter) - Mapping",
    ...
    "Keep in mind that disabling \"Unique user IDs\" means that this provider can produce user IDs that are identical to user IDs from other providers (and from other user backends if you are in non-auto-provisioning mode). This means this provider can be potentially used to authenticate as users from other providers/backends. This might be what you want to achieve. In any case, please make sure you trust this provider." : "Bitte beachten, dass das Deaktivieren von \"Eindeutigen Benutzer-IDs“ bedeutet, dass dieser Anbieter Benutzer-IDs erstellen kann, die mit Benutzer-IDs anderer Anbieter (und anderer Benutzer-Backends, wenn du dich im Modus \"Nicht automatische Bereitstellung“ befindest) identisch sind. Dies bedeutet, dass dieser Anbieter möglicherweise zur Anmeldung als Benutzer anderer Anbieter/Backends verwendet werden kann. Es kann sein, dass dies beabsichtigt ist. Bitte in jedem Fall sicherstellen, dass dieser Anbieter vertrauenswürdig ist.",
    ...
    "Do you want to allow API calls and WebDAV requests that are authenticated with an OIDC ID token or access token?" : "Sollen API-Aufrufe und WebDAV-Anfragen zugelassen werden, die mit einem OIDC-ID-Token oder Zugriffstoken authentifiziert werden?",
    ...
    "Enrich login ID token with userinfo" : "Login-ID-Token um Benutzerinformationen erweitern",
    "Fetch additional information not found in the login ID token from the userinfo endpoint. This setting is overwritten if the global enrich_login_id_token_with_userinfo option is enabled." : "Vom Userinfo-Endpunkt zusätzliche Informationen abrufen, die nicht im Login-ID-Token gefunden wurden. Diese Einstellung wird überschrieben, wenn die globale Option enrich_login_id_token_with_userinfo aktiviert ist.",
    ...
    "This will create and update the users groups depending on the groups claim in the ID token. The Format of the groups claim value should be {sample1}, {sample2} or {sample3}" : "Hierdurch werden die Benutzergruppen abhängig vom Gruppen-Claim im ID-Token erstellt und aktualisiert. Das Format des Gruppen-Claim-Werts sollte {sample1}, {sample2} oder {sample3} sein.",
