OC.L10N.register(
    ...
    "There is not such OpenID Connect provider." : "Böyle bir OpenID Connect hizmeti sağlayıcı bulunamadı.",
    ...
    "Twitter mapping" : "Twitter eşleştirmesi",
    ...
    "Use provider identifier as prefix for ids" : "Kimlik ön eki olarak sağlayıcı tanımlayıcısı kullanılsın",
    "To keep ids in plain text, but also preserve uniqueness of them across multiple providers, a prefix with the providers name is added." : "Kimliklerin düz metin olarak tutulması ve aynı zamanda birden fazla sağlayıcıda benzersizliğinin korunması için kimliklere ön ek olarak hizmet sağlayıcı tanımlayıcısı eklenir.",
    "Use group provisioning." : "Grup hazırlama kullanılsın",
    "This will create and update the users groups depending on the groups claim in the id token. The Format of the groups claim value should be {sample1} or {sample2} or {sample3}" : "Bu seçenek, kimlik kodundaki grupların isteğine bağlı olarak kullanıcı gruplarını ekler ve günceller. İstek gruplarının biçim değeri {sample1}, {sample2} veya {sample3} olabilir",
    ...
    "Do you want to allow API calls and WebDav request that are authenticated with an OIDC ID token or access token?" : "Bir OIDC kimlik kodu veya erişim kodu ile doğrulanmış API çağrılarına ve WebDav isteğine izin verilsin mi?",
