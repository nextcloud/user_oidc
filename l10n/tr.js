OC.L10N.register(
    ...
    "Failed to authenticate" : "Kimlik doğrulanamadı",
    ...
    "Keep in mind that disabling \"Unique user IDs\" means that this provider can produce user IDs that are identical to user IDs from other providers (and from other user backends if you are in non-auto-provisioning mode). This means this provider can be potentially used to authenticate as users from other providers/backends. This might be what you want to achieve. In any case, please make sure you trust this provider." : "\"Benzersiz kullanıcı kimliklerini\" etkisizleştirmenin, bu hizmet sağlayıcının diğer hizmet sağlayıcılardan (ve otomatik sağlama kipini kullanıyorsanız diğer kullanıcı arka uçlarından) kullanıcı kimlikleriyle aynı kullanıcı kimliklerini üretebileceğini unutmayın. Bu durum potansiyel olarak, bu hizmet sağlayıcının diğer hizmet sağlayıcılardan veya arka uçlardan kullanıcı olarak kimlik doğrulaması yapmak için kullanılabileceği anlamına gelir. Elde etmek istediğiniz şey bu olabilir. Her durumda, lütfen bu hizmet sağlayıcıya güvendiğinizden emin olun.",
    ...
    "Enrich login ID token with userinfo" : "Oturum açma kimliği kodu kullanıcı bilgileri ile zenginleştirilsin",
    "Fetch additional information not found in the login ID token from the userinfo endpoint. This setting is overwritten if the global enrich_login_id_token_with_userinfo option is enabled." : "Kullanıcı bilgileri uç noktasından oturum açma kimliği kodunda bulunmayan ek bilgileri alır. Global enrich_login_id_token_with_userinfo seçeneği etkinse bu ayar değiştirilir.",
